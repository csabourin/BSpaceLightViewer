    ...
    window.GraphicSelectApp = (function () {
        ...
        GraphicSelectApp.prototype.getAppData = function (callback, fileRef) {
            ...
            jqxhr.fail(
                (function (_this) {
                    return function (e) {
                        _this.d2log(
                            'ERROR!!: failed to load data from specified file. Ensure file is at that location and that JSON data is vaild. (tip: use a vaildator like: http://jsonformatter.curiousconcept.com/ )'
                        );
                        _this.d2log(e);
                    };
                })(this)
        ...
        GraphicSelectApp.prototype.buildActivity = function () {
            ...
            if (this._appData.QuestionDisplay === 'single') {
                if (this._continuousFeedback === false && this._reportFeedback === false && this._endReport === true) {
                    iText =
                        this._appData.Instructions +
                        '<br><br>' +
                        ' <span class="GS_instruct_common">' +
                        this._appData.Instruct_common +
                        ' ' +
                        this._appData.Instruct_01 +
                        ' ' +
                        this._appData.Instructions_KeyUserText +
                        '</span>';
                } else {
                    iText =
                        this._appData.Instructions +
        ...
        GraphicSelectApp.prototype.BuildFeedback = function (qNum, location) {
            ...
            if (this._appData.ShowQuestionFeedback === true) {
                if (
                    this.Questions[qNum].questionFeedback !== null &&
                    this.Questions[qNum].questionFeedback !== void 0 &&
                    this.Questions[qNum].questionFeedback !== 'none' &&
                    this.Questions[qNum].questionFeedback !== ''
                ) {
            ...
            while (i < this.Questions[qNum].answers.length) {
                if (this._allCheck[qNum][i] !== this._allCorrect[qNum][i]) {
                    if (
                        this.Questions[qNum].answers[i].feedback !== 'none' &&
                        this.Questions[qNum].answers[i].feedback !== null &&
                        this.Questions[qNum].answers[i].feedback !== void 0 &&
                        this.Questions[qNum].answers[i].feedback !== ''
                    ) {
                        j = 0;
                    }
