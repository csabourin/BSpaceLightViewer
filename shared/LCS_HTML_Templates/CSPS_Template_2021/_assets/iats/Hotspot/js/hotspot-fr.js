...
HotSpotApp.loadButtons = function () {
    ...
    HS_restartButton.setAttribute('title', "Redémarrer l'activité");
    ...
    HS_restartLabel.innerHTML = "Redémarrer l'activité";
    ...
    HS_startButton.setAttribute('title', "Démarre l'activité");
    ...
    HS_nextButton.setAttribute('title', "Redémarrer l'activité");
...
HotSpotApp.buildPostActivity = function () {
    document.getElementById('HS_header').innerHTML =
        '<h' + HotSpotApp.headingLevel + '>' + HotSpotApp.AppData.ActivityName + '</h' + HotSpotApp.headingLevel + '>';
    if (
        HotSpotApp.AppData.PostActivityText !== 'none' &&
        HotSpotApp.AppData.PostActivityText !== '' &&
        HotSpotApp.AppData.PostActivityText !== null &&
...
HotSpotApp.buildActivity = function () {
    if (HotSpotApp.AppData.ShowScenes === 1 || HotSpotApp.Scenes.length === 0) {
        document.getElementById('HS_header').innerHTML =
            '<h' + HotSpotApp.headingLevel + '>' + HotSpotApp.Scenes[HotSpotApp.sceneNum].SceneName + '</h' + HotSpotApp.headingLevel + '>';
    } else {
        document.getElementById('HS_header').innerHTML =
            '<h' +
            HotSpotApp.headingLevel +
            '>Scene ' +
            (HotSpotApp.sceneNum + 1) +
            ': ' +
            HotSpotApp.Scenes[HotSpotApp.sceneNum].SceneName +
            '</h' +
            HotSpotApp.headingLevel +
            '>';
    }
    ...
    if (
        HotSpotApp.Scenes[HotSpotApp.sceneNum].Instructions !== 'none' &&
        ...
        HS_instructions.innerHTML =
            '<p>' +
            HotSpotApp.Scenes[HotSpotApp.sceneNum].Instructions +
            '</p><p><span class="HS_instruct_common">Cliquez sur la zone active de chacune des obligations ou sélectionnez-la en la pointant, puis en tapant la touche Retour. Certaines zones vous fourniront d’autres renseignements ou comprendront des questions complémentaires. </span></p>';
        $(HS_instructions).hide().appendTo(HS_hotSpot).fadeIn();
        //HS_hotSpot.appendChild(HS_instructions);
    } else {
        HS_instructions.innerHTML =
            '<span class="HS_instruct_common">Cliquez sur la zone active de chacune des obligations ou sélectionnez-la en la pointant, puis en tapant la touche Retour. Certaines zones vous fourniront d’autres renseignements ou comprendront des questions complémentaires. </span><br/><br/>';
        $(HS_instructions).hide().appendTo(HS_hotSpot).fadeIn();
        //HS_hotSpot.appendChild(HS_instructions);
    }
    ...
    HS_progress.innerHTML =
        "Nombre d’éléments trouvés : <strong><span id='HS_completed'></span> / <span id='HS_total'>" +
        HotSpotApp.allCompleted.length +
        '</span></strong>';
    ...
    if (HotSpotApp.Scenes.length > 1 && HotSpotApp.AppData.ShowScenes > 1) {
        HS_scenetracker.innerHTML =
            'Scene: <strong>' + (HotSpotApp.sceneNum + 1) + '</strong> / <strong>' + HotSpotApp.AppData.ShowScenes + '</strong';
...
HotSpotApp.buildActivityButtons = function () {
    ...
    if (HotSpotApp.sceneNum < HotSpotApp.maxScenes - 1) {
        ...
        if (HotSpotApp.noPost === false) {
            ...
            $('.HS_buttonLabel').html("Redémarrer l'activité");
        }
    }
};
/**
...
HotSpotApp.buildHotspots = function () {
    for (i = 0; i < HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpots.length; i++) {
        ...
        tempArea.alt = 'HotSpot numéro ' + (i + 1);
        HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpots[i].desc + ' Ouvre une fenêtre contextuelle.';
        tempArea.title = 'Élément trouvé';
        ...
        tempArea.onclick = function () {
            ...
            const header = document.getElementById('HS_popUpHeader');

            header.setAttribute('tabindex', '0');
            header.focus();

            const answer = document.body.getElementsByClassName('HS_input')[0];
...
HotSpotApp.buildPopUp = function (target) {
    ...
    if (HotSpotApp.activeActivity === false) {
        ...
        close.alt = 'Fermer la fenêtre contextuelle';
        close.setAttribute('title', 'Fermer la fenêtre contextuelle');
        ...
        HS_finishButton.classList.add('HS_finishButton', 'HS_popUpButton');
        HS_finishButton.setAttribute('title', 'Fermer la fenêtre contextuelle');
        ...
        HS_finishLabel.innerHTML = 'Fermer';
        ...
        for (i = 0; i < HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent.length; i++) {
            if (HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent[i].key === id) {
                ...
                if (HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent[i].activityType === 'presentation') {
                    if (
                        HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent[i].hotSpotMedia !== 'none' &&
                ...
                // If the content is assessment type, it will build a multiple choice quiz based inside the pop up
                    ...
                    $(topLevelDocument).find('#HS_finishButton').disabled = false;
                    ...
                    HS_submitButton.setAttribute('title', 'Vérifie votre réponse');
                    ...
                    if (HotSpotApp.correctCheck[i] === true) {
                        ...
                        $('.HS_answer').each(function (index) {
                            ...
                            if (index === correctAnswer) {
                                ...
                                if (
                                    HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent[questionNum].answers[idNum].feedback !== 'none' &&
                                    ...
                                    HS_feedback.innerHTML =
                                        '<p class="HS_feedbackText"><strong class="HS_feedbackTitle">Feedback: </strong><br>' +
                                        HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent[questionNum].answers[idNum].feedback +
                                        '</p>';
    ...
    HS_questionContainer.setAttribute('role', 'dialog');
    HS_questionContainer.setAttribute('aria-modal', 'true');
    HS_questionContainer.setAttribute('aria-live', 'assertive');

    const firstFocusableElementInModal = HS_questionContainer.querySelectorAll(
        '[href], a, input, select, textarea, [tabindex]:not([tabindex="-1"]), a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])'
    )[0];
    const allFocusableElementsInModal = HS_questionContainer.querySelectorAll(
        '[href], a, input, select, textarea, [tabindex]:not([tabindex="-1"]), a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])'
    );

    const lastFocusableElementInModal = HS_questionContainer.querySelectorAll(
        '[href], a, input, select, textarea, [tabindex]:not([tabindex="-1"]), a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled]) '
    )[allFocusableElementsInModal.length - 1];
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Tab') {
            ...
            } else if (document.activeElement === lastFocusableElementInModal) {
                firstFocusableElementInModal.focus();
                e.preventDefault();
            }
        }
        if (e.key === 'Escape' && HS_questionContainer) {
            firstFocusableElementInModal.click();
        }
    });
            ...
            if (HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpotContent[i1].activityType === 'assessment') {
                ...
                if (HotSpotApp.correctCheck[i1] === true) {
                    ...
                    // If the quiz hasn't been completed yet(meaning no correctness state exists)
                        ...
                        res = str.replace('Élément trouvé', 'Élément complet');
                        document.getElementById(id).alt = res;
                        document.getElementById(id).title = 'Élément complet';
                    }
                }
...
HotSpotApp.EvaluateScore = function (questionNum, correctAnswer) {
    ...
    if (idNum === correctAnswer) {
        ...
        $('#HS_closeButton').attr('title', 'Fermer la fenêtre contextuelle');
    } else {
    ...
    switch (mediaData.type) {
        ...
        case 'audio':
            ...
            mediaDomContent.innerHTML =
                ...
                '<source src="' +
                mediaData.wav +
                '" type="audio/wav"> Your browser does not support the audio tag.';
            mediaDomObj.appendChild(mediaDomContent);
...
//     $.getScript('../../assets/facilitator-tools/Hotspot/thirdpartylib/SCORM_API_wrapper.js')
//         .done(function () {
