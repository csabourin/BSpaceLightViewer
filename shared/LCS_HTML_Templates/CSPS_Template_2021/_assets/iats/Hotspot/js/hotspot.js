...
HotSpotApp.getAppData = function (callback) {
    var jqxhr = $.getJSON(questionFile, function (data) {
        ...
        // setting the document as as the top level due to issues with styling and missing content when modal/popover loaded at top level/out of iframe
        // try {
        ...
        //     //   stylesheet2.href = getStyle('styles-d2l.min.css');
...
HotSpotApp.buildActivity = function () {
    ...
    HS_hotSpot.setAttribute('aria-live', 'assertive');
    ...
    if (
        ...
        HS_instructions.innerHTML =
            '<p>' +
            HotSpotApp.Scenes[HotSpotApp.sceneNum].Instructions +
            '</p><p><span class="HS_instruct_common">Click or tab to each hotspot and select it by clicking or hitting the enter key. Some hotspots may provide additional content or ask follow-up questions.</span></p>';
        $(HS_instructions).hide().appendTo(HS_hotSpot).fadeIn();
        //HS_hotSpot.appendChild(HS_instructions);
    } else {
        HS_instructions.innerHTML =
            '<span class="HS_instruct_common">Click or tab to each hotspot and select it by clicking or hitting the enter key. Some hotspots may provide additional content or ask follow-up questions.</span><br/><br/>';
        $(HS_instructions).hide().appendTo(HS_hotSpot).fadeIn();
        //HS_hotSpot.appendChild(HS_instructions);
    }
    ...
    HS_progress.innerHTML =
        "Items Found: <strong><span id='HS_completed'></span> / <span id='HS_total'>" + HotSpotApp.allCompleted.length + '</span></strong>';
...
HotSpotApp.buildHotspots = function () {
    for (i = 0; i < HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpots.length; i++) {
        ...
        tempArea.alt = 'Hot Spot Number ' + (i + 1) + HotSpotApp.Scenes[HotSpotApp.sceneNum].HotSpots[i].desc + ' Opens a pop up window.';
...
HotSpotApp.buildPopUp = function (target) {
    ...
    if (HotSpotApp.activeActivity === false) {
        ...
        // HS_finishButton.setAttribute('class', 'HS_finishButton');
        ...
        HS_finishLabel.innerHTML = 'Close';
